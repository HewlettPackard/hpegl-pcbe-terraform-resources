...
func (i CspMachineInstancesGetResponse_items_cspType) String() string {
    return []string{"AWS", "AZURE"}[i]
}
func ParseCspMachineInstancesGetResponse_items_cspType(v string) (any, error) {
    result := AWS_CSPMACHINEINSTANCESGETRESPONSE_ITEMS_CSPTYPE
    switch v {
        case "AWS":
            result = AWS_CSPMACHINEINSTANCESGETRESPONSE_ITEMS_CSPTYPE
        case "AZURE":
            result = AZURE_CSPMACHINEINSTANCESGETRESPONSE_ITEMS_CSPTYPE
        default:
            return nil, nil
    }
    return &result, nil
...
func (i CspMachineInstancesGetResponse_items_cspType) isMultiValue() bool {
    return false
}
